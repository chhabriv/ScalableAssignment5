...
jsonFile = open("json\level1.json","r")
...
kinds=[]
#def retrieve():
file2 = open("potfiles\level1\passwords\cracked.txt")
...
path=os.path.join("hashes\\level2\\",csname)
with open(path,"w") as tmpf:
    for hash in new_hashes:
        tmpf.write(hash+"\n")
tmpf.close()
new_shares=newball["shares"]
csname="level2"+".shares"
